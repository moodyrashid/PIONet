...
def main(args):
    ...
    with open(os.path.join(os.getcwd(),'pionet+_output.txt'), 'a') as f:
        now = datetime.now()
        ...
        model_to_load = "basemodel_301.keras"
        loaded_model = tf.keras.models.load_model(os.path.join(base_path, model_to_load))
        model = get_transfer_learning_model(loaded_model)
        # Make sure all layers are set to trainable
        ...
        for epoch in range(epochs):
            ...
            if auc_roc > best_auc_roc:
                ...
                model.save(os.path.join(model_path,f'pionet+_{experiment}_{WINDOW_SIZE}.keras'))
...
def parse_arguments(parser):
    parser.add_argument('--DATASET_NAME', type=str, default=experiment, help='Name of the fasta file')
